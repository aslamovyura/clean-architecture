...
def try_to_allocate(orderid, sku, uow, exceptions):
...
def test_concurrent_updates_to_version_are_not_allowed(postgres_session_factory):
    ...
    uow = UnitOfWork(postgres_session_factory)
    order1, order2 = random_orderid("1"), random_orderid("2")
    exceptions = []  # type: List[Exception]
    try_to_allocate_order1 = lambda: try_to_allocate(order1, sku, uow, exceptions)
    try_to_allocate_order2 = lambda: try_to_allocate(order2, sku, uow, exceptions)
    ...
    with UnitOfWork(postgres_session_factory) as uow:
